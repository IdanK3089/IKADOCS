...
public final class FilterEventHandler<S> implements EventHandler<ActionEvent> {
	...
	private static final String FILTER_DIALOG_FXML_Hebrew = "filterH.fxml";
	...
	private Popup createNewDialog() {
		...
		try {
			...
			if(Main.Hebrew==true){
				...
				resource = getClass().getResource(FILTER_DIALOG_FXML_Hebrew);

			}else{
				resource = getClass().getResource(FILTER_DIALOG_FXML);
			}
